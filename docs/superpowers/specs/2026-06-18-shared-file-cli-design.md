# shared-file — 셀프호스팅 문서 퍼블리시 CLI (설계)

- 작성일: 2026-06-18
- 상태: 설계 확정 대기 (사용자 리뷰 전)
- 배포: 공개 GitHub + npm (`shared-file`, MIT)
...
도구는 특정 도메인/계정에 묶이지 않는다. `yeonigi.com`은 작성자(사용자 1호)의 설정값일 뿐이며, 누구나 자기 도메인·버킷·배포를 지정하거나 도구가 직접 만들게 할 수 있다.

### 레퍼런스와의 차이
- htmlbook.io: 단축링크 `htmlbook.io/d/<code>`, HTML/MD 단일 문서, 웹 UI 중심, 호스팅 SaaS.
- shared-file: 단축링크 `<host>/<code>/`, 파일 또는 폴더, CLI 전용, **사용자 셀프호스팅(AWS)**.

## 2. 핵심 흐름

```
$ shared-file publish ./report.html
  → https://shared.yeonigi.com/x7Kq2a/

$ shared-file publish ./site/ --slug aws-design
  → https://shared.yeonigi.com/aws-design/

$ shared-file list           # 내가 올린 문서 목록
$ shared-file rm x7Kq2a      # 삭제 (+ 도메인 모드면 무효화)
$ shared-file open aws-design
```
...
```
브라우저  https://shared.yeonigi.com/x7Kq2a/
   ▼  Route53  (shared.yeonigi.com  A/AAAA alias → CloudFront)
...
**인프라 좌표 config** (`~/.config/shared-file/config.json`):
```json
// Mode A
{ "mode": "cloudfront", "bucket": "...", "region": "...", "distributionId": "...", "domain": "shared.yeonigi.com" }
// Mode B
{ "mode": "s3-website", "bucket": "...", "region": "...", "websiteEndpoint": "http://...s3-website-..." }
```
**우선순위**: `--bucket/--distribution/--domain/--region` 플래그 > `SHARED_FILE_*` 환경변수 > config 파일.
...
5. `~/.config/shared-file/config.json` 기록 (mode=s3-website, websiteEndpoint)

### 8.2 Terraform (Mode A, `infra/`)
`terraform.tfvars`만 채우면 `terraform apply` 한 번:
```hcl
hosted_zone_name = "yeonigi.com"     # 이미 Route53에 있는 zone (data 조회)
subdomain        = "shared"          # → shared.yeonigi.com
...
shared-file/                     # public repo
  ...
  docs/superpowers/specs/2026-06-18-shared-file-cli-design.md
```
